...
/*
 * mounthide.ko - 将 KernelSU 模块挂载从所有进程的
 *   /proc/self/mountinfo (show_mountinfo)
 *   /proc/self/mounts    (show_vfsmnt)
 *   /proc/self/mountstat (show_vfsstat)
 * 输出中隐藏，与 susfs 对 KSU 挂载的隐藏行为等价，但纯 LKM 实现。
 *
 * 原理: 这些 proc 文件的 seq_ops.show 回调是 show_mountinfo/show_vfsmnt/
 * show_vfsstat，本模块用 kprobe 注册 pre_handler，在回调执行前检查挂载
 * root 路径是否命中特征前缀(默认 /adb/modules)，命中则将 IP 重定向到
 * 一个 trampoline 直接 return 0（seq_file 语义: 该条目不输出）。
 *
 * 所有进程视图统一 -> PrivIsolated 类检测器的特征 grep 与视图一致性
 * 两重检测全部通过。模块挂载本身保留（功能不受影响）。
 *
 * 用法:
 *   insmod mounthide.ko [hide_prefix=/adb/modules;adb/modules]
 * 卸载: rmmod mounthide  (kprobe 自动恢复)
 */
